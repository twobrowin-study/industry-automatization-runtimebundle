...
import static org.activiti.cloud.runtime.ModelsContextTest.PROJECT_MODEL_DEFINITION_KEY;
...
public class ProjectModelTest {
    ...
    private final Logger logger = LoggerFactory.getLogger(ProjectModelTest.class);
    ...
    public void processStart() {
        ...
        this.processInstance = processRuntime.start(ProcessPayloadBuilder
                .start()
                .withProcessDefinitionKey(PROJECT_MODEL_DEFINITION_KEY)
                .withVariable("initiator_group", initiator_group)
                .withVariable("name", "My First Project")
                .build());
    ...
    public void processVariablesTest() {
        ...
        for (VariableInstance var : variables) {
            ...
            switch (var.getName()) {
                case "name":
                    assertThat(var.getValue().toString()).isEqualTo("My First Project");
                    break;
                ...
                case "creation_name":
                    ...
                    break;
                case "creation_type":
                    assertThat(var.getValue().toString()).isEqualTo("file");
                    break;
            }
        }
    }
    ...
    public void createFileTest() {
        ...
        processRuntime.setVariables(ProcessPayloadBuilder.setVariables(processInstance)
                ...
                .withVariable("creation_type", "file")
                .build()
        );
    ...
    public void createSubprojectTest() {
        ...
        // Усвоение имени создаваемомого подпроекта
        processRuntime.setVariables(ProcessPayloadBuilder.setVariables(processInstance)
                .withVariable("creation_name", "My First Subproject")
                .withVariable("creation_type", "subproject")
                .build()
        );

        // Выполнение задачи - создание подпроекта
        Task completedTask = taskRuntime.complete(TaskPayloadBuilder.complete()
                .withTaskId(task.getId())
                .build()
        );
        ...
        // Проверка того, что подпроект создался
        Page<ProcessInstance> fileProcessInstancePage = processRuntime.processInstances(
                Pageable.of(0, 10),
                ProcessPayloadBuilder.subprocesses(processInstance)
        );
        assertThat(fileProcessInstancePage.getTotalItems()).isEqualTo(1);
        ProcessInstance fileProcessInstance = fileProcessInstancePage.getContent().get(0);
        logger.info("> Created Subproject Process: " + fileProcessInstance);

        // Проверка того, что это действительно подпроект
        assertThat(fileProcessInstance.getProcessDefinitionKey()).isEqualTo(SUBPROJECT_MODEL_DEFINITION_KEY);
        assertThat(fileProcessInstance.getStatus()).isEqualTo(ProcessInstance.ProcessInstanceStatus.RUNNING);

        // Проверка передачи переменных подпроекту
        List<VariableInstance> variables = processRuntime.variables(ProcessPayloadBuilder
                .variables()
                .withProcessInstance(fileProcessInstance)
                .build()
